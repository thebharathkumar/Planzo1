...
// ─── Hooks ────────────────────────────────────────────────────────────────────
function useSectionReveal(direction: "up" | "left" | "right" | "scale" = "up") {
    const ref = useRef<HTMLDivElement>(null);
    useEffect(() => {
        const el = ref.current;
        if (!el) return;
        // Pick the reveal class by direction
        const revealClass = direction === "left" ? "section-reveal-left"
            : direction === "right" ? "section-reveal-right"
                : direction === "scale" ? "section-reveal-scale"
                    : "section-reveal";
        ...
        const obs = new IntersectionObserver(
            ([entry]) => {
                if (entry.isIntersecting) {
                    ...
                    // Stagger children — support multiple classes
                    const children = el.querySelectorAll(
                        ".stagger-child, .stagger-scale, .stagger-left, .stagger-right, .stagger-pop"
                    );
...
function HeroFeaturedCard({ event }: { event: Event }) {
    return (
        <Link to={`/events/${event.id}`} style={{ textDecoration: "none" }}>
            <div className="stagger-child card-lift relative rounded-3xl overflow-hidden group cursor-pointer" style={{ height: 460 }}>
                <img
                    src={event.image}
                    alt={event.title}
...
function SideFeaturedCard({ event }: { event: Event }) {
    return (
        <Link to={`/events/${event.id}`} style={{ textDecoration: "none" }}>
            <div className="stagger-right card-lift relative rounded-2xl overflow-hidden group cursor-pointer" style={{ height: 218 }}>
...
function EventCard({ event, index }: { event: Event; index: number }) {
    return (
        <div
            className="stagger-pop card-lift"
...
export function EventDiscovery() {
    ...
    const filtered = useMemo(() => {
        ...
        return events;
    }, [allEvents, search, category, sortBy]);

    const featured = allEvents.filter(e => e.featured).slice(0, 3);

    // Map center calculation
    const mapCenter = useMemo<[number, number]>(() => {
        if (allEvents.length === 0) return [37.7749, -122.4194];
        const avgLat = allEvents.reduce((s, e) => s + e.lat, 0) / allEvents.length;
        const avgLng = allEvents.reduce((s, e) => s + e.lng, 0) / allEvents.length;
        return [avgLat, avgLng];
    }, [allEvents]);

    // Section refs
    const statsRef = useSectionReveal("scale");
    const featuredRef = useSectionReveal("up");
    const mapRef = useSectionReveal("left");
    const categoriesRef = useSectionReveal("up");
    const gridRef = useSectionReveal("up");
    const ctaRef = useSectionReveal("scale");
    ...
    return (
        <div style={{ paddingTop: 68, position: "relative", zIndex: 0 }}>
            ...
            <div className="relative overflow-hidden" style={{ minHeight: 540, background: "linear-gradient(165deg, #1a0a00 0%, #2d1400 40%, #451a04 100%)" }}>
                ...
                <div className="absolute rounded-full blur-3xl orb-drift pointer-events-none" style={{ width: 500, height: 500, top: -200, right: -100, background: "rgba(249,115,22,0.18)", animationDuration: "14s" }} />
                <div className="absolute rounded-full blur-3xl orb-drift pointer-events-none" style={{ width: 350, height: 350, bottom: -100, left: -80, background: "rgba(239,68,68,0.13)", animationDuration: "18s", animationDelay: "-5s" }} />
                <div className="absolute rounded-full blur-2xl orb-drift pointer-events-none" style={{ width: 180, height: 180, top: 100, left: "40%", background: "rgba(251,191,36,0.1)", animationDuration: "10s", animationDelay: "-3s" }} />
                ...
                <div ref={mapRef} className="section-reveal py-12">
                    <div className="flex items-end justify-between mb-6">
                        ...
                        <div className="flex items-center gap-1.5 text-xs font-medium" style={{ color: "#78716c" }}>
                            <MapPin size={12} style={{ color: "#f97316" }} /> {allEvents.length} events shown
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                        {CATEGORIES.map(({ name, icon: CatIcon }, i) => (
                            <button
                                ...
                                className="stagger-pop flex items-center gap-1.5 px-4 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 hover:-translate-y-0.5 active:scale-95"
                                style={{
